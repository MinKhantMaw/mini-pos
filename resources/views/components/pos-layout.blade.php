...
<body class="min-h-screen bg-slate-100 text-slate-800">
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-200">
        <div class="mx-auto flex min-h-screen max-w-7xl flex-col lg:flex-row">
            <aside
                class="w-full border-b border-slate-200 bg-slate-900 p-4 text-slate-100 lg:w-72 lg:border-b-0 lg:border-r">
                <div class="flex items-center justify-between lg:block">
                    <div>
                        <p class="text-xs uppercase tracking-[0.35em] text-slate-400">Mini POS</p>
                ...
                <nav class="mt-6 space-y-2">
                    ...
                    <a href="{{ route('invoices.show', optional(App\Models\Sale::latest('sale_date')->first())->id ?? 0) }}"
                ...
                <header class="border-b border-slate-200 bg-white/90 px-4 py-4 shadow-sm backdrop-blur sm:px-6">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-3">
                            <a href="{{ route('products.index') }}"
                                class="rounded-full border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50">Products</a>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button
                                    class="rounded-full bg-slate-900 px-3 py-2 text-sm font-medium text-white">Logout</button>
                            </form>
                        </div>
                    </div>
                </header>
